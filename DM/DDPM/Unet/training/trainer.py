...
def train_one_epoch(model, sd, loader, optimizer, scaler, loss_fn, epoch, current_round, total_rounds, total_epochs, user_id,
    ...
    with tqdm(total=len(loader), dynamic_ncols=True) as tq:  # 创建一个进度条
        tq.set_description(f"Train user{user_id + 1} :: round {current_round}/{total_rounds} :: Epoch: {epoch}/{total_epochs}")  # 设置进度条的描述
